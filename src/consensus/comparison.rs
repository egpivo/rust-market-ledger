...
pub struct ConsensusMetrics {
    ...
    // Extended metrics from trilemma paper (arXiv:2505.03768)
    // Degree of Decentralization (DoD)
...
pub async fn benchmark_consensus_strategy(
    ...
    // Calculate extended trilemma metrics
    let availability = if !blocks.is_empty() {
        (committed_count as f64 / blocks.len() as f64) * 100.0
    } else {
        0.0
    };
    ...
    let fault_tolerance = if strategy.requirements().requires_majority {
        ...
        // Simplified: assume 4 nodes, can tolerate 1 faulty = 0.25
        0.25
    } else {
    ...
    ConsensusMetrics {
        ...
        wealth_distribution,
        availability,
        confirmation_latency_ms: avg_latency,
        max_throughput_tps: throughput,
        cost_of_attack,
        fault_tolerance,
        reliability,
        stale_block_rate,
