...
async fn main() {
    ...
    let strategies: Vec<Arc<dyn ConsensusStrategy>> = vec![
        ...
        Arc::new(ConsensusAlgorithmAdapter::new(
            Arc::new(gossip::GossipConsensus::new(0, 3, 2))
        )),
        Arc::new(ConsensusAlgorithmAdapter::new(
            Arc::new(eventual::EventualConsensus::new(0, 100, 1))
        )),
    ];
    
    println!("Strategies:");
    for (i, strategy) in strategies.iter().enumerate() {
        println!("  {}. {} - {}", i + 1, strategy.name(), strategy.requirements().description);
    }
    println!();
    ...
    println!("Analysis:");
    println!("  {} out of {} strategies committed", committed_count, results.len());
    
    if let Some(fastest) = results.iter().min_by_key(|r| r.execution_time_ms) {
        println!("  Fastest: {} ({} ms)", fastest.strategy_name, fastest.execution_time_ms);
    }
    
    if let Some(slowest) = results.iter().max_by_key(|r| r.execution_time_ms) {
        println!("  Slowest: {} ({} ms)", slowest.strategy_name, slowest.execution_time_ms);
    }
    println!();
}
