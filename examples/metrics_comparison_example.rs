...
async fn main() {
    ...
    println!("Executing benchmark ({} blocks per strategy)...", blocks.len());
    println!();
    ...
    for metric in &metrics {
        ...
        println!("  Throughput: {:.2} blocks/sec", metric.throughput_blocks_per_sec);
        println!("  Latency: min={}ms, max={}ms, avg={:.2}ms", 
            metric.min_latency_ms, metric.max_latency_ms, metric.avg_latency_ms);
        println!("  Commit Rate: {:.2}% ({}/{})", 
        ...
        println!("  Data Integrity: {}", 
            if metric.data_integrity_maintained { "Maintained" } else { "Compromised" });
        println!();
    }
    ...
    let no_consensus = metrics.iter().find(|m| m.strategy_name == "No-Consensus (Single Node)");
    let pbft = metrics.iter().find(|m| m.strategy_name == "PBFT");
    
    if let (Some(no_cons), Some(pbft_metric)) = (no_consensus, pbft) {
        let throughput_diff = no_cons.throughput_blocks_per_sec / pbft_metric.throughput_blocks_per_sec.max(0.01);
        println!("Throughput:");
        println!("  No-Consensus: {:.2} blocks/sec", no_cons.throughput_blocks_per_sec);
        println!("  PBFT: {:.2} blocks/sec", pbft_metric.throughput_blocks_per_sec);
