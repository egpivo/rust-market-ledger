...
async fn main() {
    ...
    println!("Nodes: {}, Majority: {}/{}", total_nodes, majority, total_nodes);
    println!();
    ...
    match strategy.execute(&block).await {
        Ok(Some(committed_block)) => {
            ...
            println!("Block committed: latency={:.2}ms, index={}, votes={}/{}", 
                elapsed.as_secs_f64() * 1000.0, committed_block.index, majority, total_nodes);
            println!();
            println!("Advantages:");
